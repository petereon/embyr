...
/**
 * Create an isolated Firestore Lite client connected to the local firstyr REST port.
 *
 * The Lite SDK uses plain HTTP REST for all operations — no gRPC, no BrowserChannel.
 * This exercises firstyr's grpc-gateway REST transport path.
 */
export function makeDb() {
  const appName = crypto.randomUUID();
  const app = initializeApp({ projectId: 'p' }, appName);
  const db = getFirestore(app);
  connectFirestoreEmulator(db, '127.0.0.1', parseInt(process.env.FIRSTYR_REST_PORT, 10));
  return { app, db };
}
